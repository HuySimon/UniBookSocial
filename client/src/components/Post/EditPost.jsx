...
import { editPostSchema } from '../../validations/PostValidation';
...
const EditPost = ({ post, handleEditPost, isVisibleEditPost }) => {
	...
	return (
		<>
				...
				<div className="w-full h-full flex flex-col">
					...
					<form onSubmit={handleSubmit(onSubmit)} className="flex flex-col">
									...
									{selectedFile === null ? (
										...
										<>
											...
											<AiOutlineClose
												...
												size={40}
												className="text-white bg-black p-3 rounded-full absolute top-2 right-2 transition-all hover:bg-gray-800"
												cursor={'pointer'}
											/>
											<p className="text-sm text-red-600 absolute left-0 -bottom-5 truncate">
												{errors.mainImage?.message}
							...
							<div className="flex justify-between items-center gap-5">
								<div className="w-full flex flex-col mb-3 relative">
									...
									<select
										...
										defaultValue={post.isGeneralSubject}
						...
						<div className="px-4 py-2 mb-2">
							<button
								type="submit"
								className={`w-full p-2 bg-primary-900 text-center rounded-lg text-white`}
							>
								Save
							</button>
						</div>
			...
			<motion.div className="fixed w-[calc(100%+250px)] top-0 left-0 h-screen bg-black/50 z-[11]"></motion.div>
			<AiOutlineClose
				onClick={() => {
					handleEditPost();
				}}
