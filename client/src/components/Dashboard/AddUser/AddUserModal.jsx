...
function AddUserModal({ onClose }) {
	...
	return (
		...
		>
			<motion.div ref={modalRef} className="relative w-full max-w-2xl max-h-full">
				{/* <!-- Modal content --> */}
				<form onSubmit={handleSubmit(handleSaveClick)} className="relative bg-white rounded-lg shadow">
							...
							<div className="col-span-6 sm:col-span-3 relative mb-4">
								...
								<p className="text-sm text-red-400 absolute top-full">{errors.password?.message}</p>
								{
									!eye ? (
										<AiOutlineEye
											onClick={() => { setEye(!eye) }}
											size={20} className='absolute top-[58%] right-2' />
									) : (
					...
					<div className="flex items-center p-6 space-x-2 border-t border-gray-200 rounded-b">
						<button
							...
							type="submit"
							className="text-white bg-primary-900 hover:bg-primary-800 transition-all focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center"
						>
							Add user
						</button>
					</div>
